#!/usr/bin/env python3
"""Cross-platform launcher for the analysis scripts.

Run it with whatever Python you have and it sorts the rest out:

    python run.py                        EDA.py, the default
    python run.py phase_a.py --full      any script, arguments passed through
    python run.py phase_b.py             (Windows: py run.py ...)

It looks for an interpreter that can import everything the scripts need. If none
can, it builds a project-local virtual environment in .venv/ and installs
requirements.txt into it, then runs there. VS Code auto-detects a .venv/ in the
workspace root, so creating one also fixes the run button without anyone having
to hardcode an interpreter path.
...
DEFAULT_SCRIPT = "EDA.py"

# Import names, which differ from the pip names for scikit-learn. `requests`
# and `pyarrow` are only needed by the fetch/parquet path, but a single probe
# keeps the interpreter choice from depending on which script is being run.
MODULES = ["numpy", "pandas", "scipy", "sklearn", "matplotlib", "requests", "pyarrow"]
...
def main():
    ...
    script, script_args = (sys.argv[1], sys.argv[2:]) if len(sys.argv) > 1 \
        else (DEFAULT_SCRIPT, [])
    ...
    # cwd=HERE so the script finds its data and writes its outputs beside itself.
    return subprocess.call([python, path] + script_args, cwd=HERE)
